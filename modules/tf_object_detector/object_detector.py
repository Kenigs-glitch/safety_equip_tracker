...
from modules_helper.deep_sort_tracker_helper.deep_sort_tracker_helper import \
    deep_sort_tracker_helper, are_segments_intersected
...
class object_detector:
    ...
    def __init__(self):
        ...
        self.regions = []
        self.colors = [(255, 255, 255), (0, 255, 255), (0, 0, 255), (255, 0, 255), (255, 255, 0), (127, 21, 173),
                       (222, 230, 31), (4, 71, 252),
                       (131, 243, 96), (91, 240, 83), (28, 185, 175), (147, 147, 43), (74, 27, 51), (246, 208, 101),
                       (122, 199, 200), (2, 198, 8),
                       (164, 199, 229), (58, 249, 131), (211, 81, 10), (153, 174, 136), (45, 254, 224), (80, 113, 77),
                       (70, 243, 38), (61, 237, 61), (188, 90, 221),
                       (67, 48, 57), (224, 37, 54), (112, 172, 36), (28, 34, 63), (126, 187, 65), (206, 146, 136),
                       (207, 145, 223), (52, 225, 238), (92, 20, 58),
                       (23, 203, 91), (84, 97, 204), (49, 201, 238), (182, 73, 121), (166, 138, 149), (199, 106, 9),
                       (123, 218, 135), (51, 204, 171), (128, 25, 70),
                       (93, 53, 224), (163, 102, 193), (78, 36, 146), (115, 25, 218), (204, 164, 135), (162, 249, 15),
                       (8, 41, 0), (142, 55, 73), (168, 254, 246),
                       (29, 236, 46), (8, 91, 254), (247, 250, 29), (13, 225, 200), (238, 211, 112), (81, 242, 180),
                       (111, 246, 73), (211, 220, 148), (230, 238, 186),
                       (186, 186, 73), (53, 247, 82), (49, 101, 45), (219, 24, 146), (227, 32, 242), (119, 19, 235),
                       (190, 220, 68), (13, 127, 195), (140, 22, 37),
                       (21, 249, 203), (140, 161, 225), (193, 23, 82), (188, 11, 132), (57, 38, 218), (181, 188, 53),
                       (217, 107, 1), (222, 167, 91), (13, 251, 188), (114, 70, 37)]

    def process(self, frame):
        line_crossed_objects = {}
        raw_frame = frame
        module_settings = {
        ...
        try:
            ...
            return draw_script, ms, fps, eff_boxes
        ...
        # line_crossed_objects = self.tracker_helpers[cam_id].check_line_crossing(module_settings['lines'])
        ...
        for track in self.tracker_helpers[cam_id].tracker.tracks:
            ...
            if track.class_ in list(self.classes.values()):
                color = self.colors[
                    list(self.classes.values()).index(track.class_)]  # [eff_labels.index(track.class_)]]
            else:
                color = None
            ...
            box_image = frame[box[1]:box[3], box[0]:box[2]]
            box_image = box_image / 255.
            box_image = cv2.resize(box_image, (299, 299))
            image_np = box_image
            hardhat_result = self.hardhat_classifier.run(image_np)

            if hardhat_result == 'nothing':
                track.safety_equip_history.append(0)
            else:
                track.safety_equip_history.append(1)

            if len(track.safety_equip_history) > 5:
                track.safety_equip_history = track.safety_equip_history[-5:]
            track.safety_equip_class['Hardhat'] = False if sum(track.safety_equip_history) / len(track.safety_equip_history) < 0.5 else True

            if module_settings['draw_name'] == 1:
                label = '{}, hardhat'.format(track.class_) if track.safety_equip_class[
                    'Hardhat'] else '{}, nothing'.format(track.class_)
                if module_settings['draw_confid'] == 1:
                    label = '{} {:.2f}%'.format(track.class_,
                                                track.confidence * 100)
                draw_script.add_label(Label(label, (box[0] + 10, box[1] + 35), color=(0, 255, 0)))
        return draw_script, ms, fps
