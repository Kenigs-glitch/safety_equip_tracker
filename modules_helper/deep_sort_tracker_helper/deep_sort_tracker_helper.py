...
def are_segments_intersected(segment1, segment2):
    p1, p2 = segment1
    q1, q2 = segment2
    if sign((p2[0] - p1[0]) * (q1[1] - p1[1]) - (p2[1] - p1[1]) * (q1[0] - p1[0])) == 0 or sign(
            (p2[0] - p1[0]) * (q2[1] - p1[1]) - (p2[1] - p1[1]) * (q2[0] - p1[0])) == 0:
        return True
    elif sign((p2[0] - p1[0]) * (q1[1] - p1[1]) - (p2[1] - p1[1]) * (q1[0] - p1[0])) == sign(
            (p2[0] - p1[0]) * (q2[1] - p1[1]) - (p2[1] - p1[1]) * (q2[0] - p1[0])):
        return False
    else:
        return True
...
class deep_sort_tracker_helper:
    ...
    #def check_line_crossing(self, lines):
    #    line_crossed_objects = {}
    #    for track in self.tracker.tracks:
    #        #track.direct_move_state = DirectMoveState.Green
    #        try:
    ...
    #            if are_segments_intersected(lines[0]['points'], last_track_segment):
    #                if not track.first_line_crossed[0]:
    #                    track.first_line_crossed[0] = True
    #                   print('Track {} has crossed the first line'.format(track.track_id))
    #                   line_crossed_objects[track.track_id] = 1
    #            if are_segments_intersected(lines[1]['points'], last_track_segment):
    #                if not track.second_line_crossed[0]:
    #                    track.second_line_crossed[0] = True
    #                    print('Track {} has crossed the second line'.format(track.track_id))
    #                   line_crossed_objects[track.track_id] = 2
    #        finally:
    #            return line_crossed_objects
    ...
    def generate_events(self, module_settings, frame):
            ...
            if is_in_region != track.is_in_forbidden_region:
                analytic_journal.append(Analytics(module=Module.object_detector,
                    ...
                    is_in_region else SubType.move_out, cam_id=cam_id,
                                                  ...
                                                  link_id='vnezapniy_id'))  # TODO(Ruslan K.): указать ID
                    ...
                    if are_segments_intersected(line['points'], last_track_segment):
                        ...
                        if get_point_pos_to_line(p0, *line['points']) < 0:
                            if track.track_id not in self.DetectedIDx:
                                ...
                                messtr = "Object detector: Object number {} intersect line A->B: {}".format(
                                    track.track_id, self.counter_lines[line_id][0])
                            ...
                            if track.track_id not in self.DetectedIDx:
                                ...
                                messtr = "Object detector: Object number {} intersect line B->A: {}".format(
                                    track.track_id, self.counter_lines[line_id][1])
                ...
                if region_type == RegionType.current_count_region or \
                    ...
                    if is_counter_changed:
                        ...
                        if region_type == RegionType.current_count_region:
                            ...
                            messtr = "Object detector: The number of obj in the region {} has become {}".format(
                                region['title'], self.counter_regions[region_id])
        ...
        for j in self.print_result:
            ...
            if j != '':
                cv2.rectangle(overlay, (text_coord[0] - 5, text_coord[1] - 2 * text_height - 5),
                              (text_width[0] + 10, text_coord[1] + text_height), (0, 0, 0), -1)
                opacity = 0.5
